...
/* ==========================================================================
   FEATURE: HUGGING FACE AI IMAGE API ENGINE (FLUX.1 & STABLE DIFFUSION 3.5)
   ========================================================================== */
...
function saveHuggingFaceApiKey() {
  ...
  const val = input.value.trim();
  if (!val) {
    if (msgArea) msgArea.innerHTML = `<span style="color:#ef4444; font-weight:bold;">❌ Please paste a valid Hugging Face Access Token (hf_...)</span>`;
    alert("Please enter a valid Hugging Face Token!");
    return;
  }

  localStorage.setItem("huggingface_api_token", val);
...
function updateHuggingFaceApiKeyStatusUI(forceSync = false) {
  ...
  const badge = document.getElementById("huggingface-key-status-badge");
  const input = document.getElementById("setting-huggingface-key");
  ...
  if (badge) {
    if (key) {
      badge.innerHTML = `<span style="background:rgba(236,72,153,0.15); color:#ec4899; border:1px solid rgba(236,72,153,0.4); padding:3px 10px; border-radius:12px; font-weight:bold; font-size:11px;">🤗 Active FLUX.1 / SD Engine</span>`;
    } else {
      badge.innerHTML = `<span style="color:#aaa; font-size:11px;">Optional Image Engine</span>`;
    }
  }
}
...
async function testHuggingFaceApiConnection() {
  ...
  if (input && input.value.trim()) {
    localStorage.setItem("huggingface_api_token", input.value.trim());
    updateHuggingFaceApiKeyStatusUI(true);
  }

  const token = getHuggingFaceApiKey();
  if (!token) {
    if (msgArea) msgArea.innerHTML = `<span style="color:#ef4444;">Please enter your Hugging Face Token (hf_...) first!</span>`;
    alert("Please paste your Hugging Face Token first!");
    return;
  }

  if (msgArea) msgArea.innerHTML = `<span style="color:#fbbf24;">Connecting to Hugging Face Inference API...</span>`;

  try {
    const dataUrl = await callHuggingFaceImageAPI("A high resolution scientific diagram of a plant cell");
    if (msgArea) msgArea.innerHTML = `<span style="color:#ec4899; font-weight:bold;">✅ Hugging Face FLUX.1 Engine Connected!</span>`;
    alert(`🎉 Hugging Face API Connection Successful!\n\nImage generation verified.`);
  } catch (err) {
    if (msgArea) msgArea.innerHTML = `<span style="color:#ef4444; font-weight:bold;">❌ Connection Failed: ${escapeHTML(err.message)}</span>`;
...
async function callHuggingFaceImageAPI(prompt) {
  ...
  for (const m of modelsToTry) {
    const urls = [
      ...
      `https://router.huggingface.co/models/${m}`,
    ...
    for (const url of urls) {
      try {
        const res = await fetch(url, {
          ...
          body: JSON.stringify({
            inputs: `${prompt}, highly detailed NCERT textbook scientific illustration, 8k resolution, crisp labels`
          })
        });
        ...
        if (!res.ok) {
          ...
          console.warn(`[HF API] Endpoint ${url} error:`, msg);
          lastError = new Error(`HF Model ${m} (${url}): ${msg}`);
          continue;
        }
        ...
        lastError = e;
        console.warn(`[HF API] Network error on ${url}:`, e.message);
      }
    }
  }

  throw lastError || new Error("All Hugging Face image endpoints failed.");
}
  ...
  for (const m of modelsToTry) {
    try {
      ...
      if (!res.ok) {
        ...
        console.log(`[OpenRouter Failover] Model ${m} unavailable (${msg}). Trying next fallback model...`);
...
const SYSTEM_PROMPTS = {
  physics: `You are an expert NTA NEET Physics tutor. Provide concise, clear, NCERT-syllabus aligned solutions. For numericals, show key formulas and direct calculation steps clearly. Avoid unnecessary multi-page derivations or out-of-syllabus calculations unless explicitly requested. End with a 1-sentence "Quick Recall Point".`,
  chemistry: `You are an expert NTA NEET Chemistry tutor (Physical, Organic, Inorganic). Provide concise, clear, NCERT-focused answers with balanced equations or key mechanisms. Format chemical formulas properly (e.g. H₂O, Fe³⁺). Avoid out-of-syllabus theory. End with a 1-sentence "Quick Recall Point".`,
  biology: `You are an expert NTA NEET Biology tutor. Provide crisp, concise, NCERT line-by-line factual answers. Explain concepts directly in clean bullet points strictly aligned with NCERT Class 11/12 textbooks. Avoid out-of-syllabus extra theory. End with a 1-sentence "Quick Recall Point".`
};
...
async function generateAiImageWithOpenRouter(prompt, containerOrEl, options = {}) {
  ...
  statusEl.style.cssText = "padding:12px; margin-top:10px; border:1px solid rgba(236,72,153,0.4); background:rgba(236,72,153,0.06); text-align:center; font-size:12px;";
  statusEl.innerHTML = `🖼️ <strong>Generating High-Res Image with Hugging Face FLUX.1 Engine...</strong>`;
  ...
  let imageUrl = "";
  let engineName = "📖 Real NCERT Textbook Figure";

  if (hfToken) {
    try {
      imageUrl = await callHuggingFaceImageAPI(prompt);
      engineName = "🤗 Hugging Face AI (FLUX.1 / SD 3.5)";
    } catch (hfErr) {
      console.warn("Hugging Face API error, falling back to NCERT Figure Database:", hfErr);
      imageUrl = getRealBookDiagramImageUrl(prompt);
    }
  } else {
    imageUrl = getRealBookDiagramImageUrl(prompt);
  }
  ...
  statusEl.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; border-bottom:1px solid rgba(236,72,153,0.3); padding-bottom:6px;">
      <span style="font-size:12px; font-weight:bold; color:#ec4899;">${engineName} — ${escapeHTML(prompt)}</span>
      <div style="display:flex; gap:6px;">
    ...
    <div style="text-align:center; padding:10px; background:rgba(0,0,0,0.4); border-radius:8px;">
      <img src="${imageUrl}" alt="${escapeHTML(prompt)}" style="max-width:100%; max-height:480px; border-radius:8px; border:1px solid rgba(255,255,255,0.15); cursor:pointer; object-fit:contain; background:#fff; padding:6px;" onclick="window.open('${imageUrl}', '_blank')" title="Click to view full resolution image" />
      <p style="font-size:11px; color:#aaa; margin-top:8px;">💡 <em>High Resolution Diagram. Click image to expand to full screen.</em></p>
    </div>
  `;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const saveHfBtn = document.getElementById("btn-save-huggingface-key");
  if (saveHfBtn) saveHfBtn.onclick = saveHuggingFaceApiKey;

  const testHfBtn = document.getElementById("btn-test-huggingface-key");
  if (testHfBtn) testHfBtn.onclick = testHuggingFaceApiConnection;

  const removeHfBtn = document.getElementById("btn-remove-huggingface-key");
  if (removeHfBtn) removeHfBtn.onclick = removeHuggingFaceApiKey;
